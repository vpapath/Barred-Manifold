...
def lyapunov_spectrum(
    ...
    for _ in range(n_steps):
        ...
        Q = Q_new  # BUGFIX: propagate the renormalized tangent map across iterations.
                   # Without this line, Q is never updated past its initial value
                   # (np.eye(n)), so every step recomputes (I + dt*J) from scratch
                   # instead of accumulating the true tangent flow. This silently
                   # produces qualitatively wrong exponents (e.g. lambda_1 < 0 for
                   # the classic chaotic rho=28 case, where it must be positive).
        state = state_new
        t += dt
...
def characterize_clinical_regimes(verbose: bool = True) -> dict:
    ...
    for name, rho, atype in REGIMES:
        ...
        if sorted_exp[0] < 0:
            # All exponents negative: stable fixed point / non-chaotic attractor,
            # Lyapunov dimension is 0 by definition. The general formula below
            # (via searchsorted) mishandles this edge case and can return a
            # spurious small positive value; handle it explicitly.
            ky_dim = 0.0
        else:
